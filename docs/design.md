...
- **Recent practice:** one heading and up to two cards, named by feel. _Pick up where you left off_ continues an interrupted session or plays the latest one again. _Try next_ is the recommendation, when it is something else. _Full history_ opens the records in place (never copied into the tool panel): when, the preset name or title, how far the session got ("stopped at chord by chord", never how well), a mini step rail and your notes. An overflow menu holds the revisit interval, export, import and clear.
...
A completed session shows a two-column **summary card**: _Session complete_, the length played, the assignment, tempo and time practised, _Again_, _Same shapes, new key_ and _Back to the stage_, and beside them each step you spent time on, with what it involved ("Left hand · looped bar 3 · half speed"), and _Notes for next time_, saved to that session's history record. There are no scores and no streaks.
