...
The home screen is the **Stage**. It leads with how the assignment feels ("Bright and hopeful."), what each hand does, and a quieter "In theory" line with _Why it works_ on request. `coach/feel.ts` derives that wording from the Score the player hears: a mood from the mode, how busy the left hand is, the right hand's phrase length, contour and syncopation, and the harmony's journey from each chord's distance from home. It never uses theory vocabulary, and custom progressions get equally truthful wording. Below the brief, one keybed holds the practice controls, the piano roll, the keyboard and a legend; Recent practice follows. Everything else (changing the assignment, shapes and scale, sound, MIDI, star and share) opens from the header into a side panel, or into a bottom sheet on narrow screens. The approved mock-up is kept in [`docs/design-reference/`](docs/design-reference/phase-1-mockup.html); see [`docs/design.md`](docs/design.md).

**Start 5 minutes** runs a guided session: the whole thing, left hand, right hand, chord by chord, note by note, then free improvisation over the loop, with one line of instruction for each step. Time is shared 60/45/45/60/45/45 seconds, and a step whose time is up moves on at the next bar line rather than mid-bar. Sessions can also be 2 or 10 minutes, or untimed. While a session runs or is paused the Stage goes into **focus mode**: the tools, the brief, Recent practice and keyboard options step aside. The clock, _Next step_ and _End session_ take the header; the step rail, the step and its instruction take the top of the Stage; the transport's main button pauses and resumes; and the keyboard, timeline, views, hands, speed and bar looping all stay usable. It is state-driven CSS on `data-session-status`, so nothing remounts: a held MIDI note and the loaded piano survive it. The summary card shows every step's time and can save notes to the session's record in Recent practice. It offers _Again_, _Same shapes, new key_, or _Back to the stage_.
...
**Star it, reopen it tomorrow.** Star, Share link and the Starred list are under the star in the header; the share button beside it shares or copies the link directly. Share uses the device's share sheet where there is one, else copies the link. `application/library.ts` keeps starred assignments, the last assignment, the tempo, and the label and session-length preferences in this browser's `localStorage`, stored as share fragments and read back through the same decoder. Opening the app without a link returns to the last assignment.
