...
The home screen is the **Stage**. It leads with how the assignment feels ("Bright and open, a loop that circles back to the start."), and the theory sentence sits underneath as the reason it works. `coach/feel.ts` derives that wording from the Score the player hears: a mood from the mode, the harmony's journey from each chord's distance from home, how busy the left hand is, and the right hand's phrase length, contour and syncopation. It never uses theory vocabulary, and custom progressions get equally truthful wording. Below the headline are the keyboard, the timeline, the practice controls and Recent practice. Everything else (changing the assignment, the scale reference, sound, MIDI, star and share) opens from the header's tool rail into a side panel, or into a bottom sheet on narrow screens.

**Start 5 minutes** runs a guided session: the whole thing, left hand, right hand, chord by chord, note by note, then free improvisation over the loop, with one line of instruction for each step. Time is shared 60/45/45/60/45/45 seconds, and a step whose time is up moves on at the next bar line rather than mid-bar. Sessions can also be 2 or 10 minutes, or untimed. While a session runs or is paused the Stage goes into **focus mode**: the tool rail, tool panel, Recent practice and keyboard options step aside. A large clock, the step rail, the instruction and Pause/Next/End take the top of the page, and the keyboard, timeline, views, hands, speed and bar looping all stay usable. It is state-driven CSS on `data-session-status`, so nothing remounts: a held MIDI note and the loaded piano survive it. The summary card shows what was covered and can save a note to the session's record in Recent practice. It offers the same assignment again, the same material in a new key, or Done.
...
**Star it, reopen it tomorrow.** Star, Share link and the Starred list are under _Star & share_ in the tool rail. Share uses the device's share sheet where there is one, else copies the link. `application/library.ts` keeps starred assignments, the last assignment, the tempo, and the label and session-length preferences in this browser's `localStorage`, stored as share fragments and read back through the same decoder. Opening the app without a link returns to the last assignment.
